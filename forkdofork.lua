...
by Nebula Softworks
Prerelease Beta 5.03Meow
]]

--// SECTION : Core Variables

local Release = "Starlight Interface forked by All iHave"
local debugV = false
...
local Fonts = {
	...
	-- Kanit — used for the player display name
	Kanit    = Font.fromId(12187373592, Enum.FontWeight.Regular),
}
--//ENDSUBSECTION
...
function Starlight:CreateWindow(WindowSettings)
	...
	do
		...
		mainWindow.Sidebar.Player.Header.FontFace = Fonts.Kanit
		...
		-- Enable with PlayerStatus = true. Change at runtime: Starlight.Window.SetPlayerStatus("AFK")
		if WindowSettings.PlayerStatus then
			local subheader = mainWindow.Sidebar.Player.subheader
			subheader.RichText = true
			subheader.TextColor3 = Color3.new(1, 1, 1) -- RichText controls colours internally

			-- Returns a brighter version of the theme accent mid-colour
			local function getBrightAccentHex()
				...
				local bright = Color3.fromHSV(h, math.min(s * 1.1, 1), 1)
				return string.format("%02X%02X%02X",
					math.floor(bright.R * 255),
					math.floor(bright.G * 255),
					math.floor(bright.B * 255)
				)
			end

			-- Returns the neutral label colour as hex
			local function getNeutralHex()
				local c = Starlight.CurrentTheme.Foregrounds.Medium
				return string.format("%02X%02X%02X",
					math.floor(c.R * 255),
					math.floor(c.G * 255),
					math.floor(c.B * 255)
			...
			local currentStatusText = "Loading"

			local function rebuildLabel()
				subheader.Text = string.format(
					'<font color="#%s">Status:</font> <font color="#%s" face="GothamBlack">%s</font>',
					getNeutralHex(),
					getBrightAccentHex(),
					currentStatusText
				)
			...
			-- Re-render when theme changes so colours stay in sync
			themeEvent.Event:Connect(rebuildLabel)

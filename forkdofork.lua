...
function Starlight:CreateWindow(WindowSettings)
	...
	do
		...
		if WindowSettings.PlayerStatus then
			...
			-- Dynamic value label — sibling of subheader (same parent) so ClipsDescendants
			-- on the subheader never clips it. Position is calculated using AbsolutePosition
			-- for pixel-perfect horizontal + vertical alignment.
			...
			statusValue.Size = UDim2.fromOffset(0, subheader.AbsoluteSize.Y)
			...
			-- Gradient: accent colour of the active theme
			...
			-- Neon glow: two UIStroke layers stacked for a vivid bloom effect.
			-- Inner stroke — thin, near-opaque: the sharp bright core
			local glowInner = Instance.new("UIStroke")
			glowInner.Thickness = 0.6
			glowInner.Transparency = 0.1
			glowInner.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual
			glowInner.Parent = statusValue

			-- Outer glow frame behind the text — blurred halo via ImageLabel trick
			local glowHalo = Instance.new("ImageLabel")
			glowHalo.Name = "GlowHalo"
			glowHalo.BackgroundTransparency = 1
			glowHalo.Image = "rbxassetid://5028857084" -- radial gradient asset
			glowHalo.ImageTransparency = 0.55
			glowHalo.ScaleType = Enum.ScaleType.Stretch
			glowHalo.Size = UDim2.new(1, 20, 1, 14)
			glowHalo.Position = UDim2.new(0, -10, 0, -7)
			glowHalo.ZIndex = statusValue.ZIndex - 1
			glowHalo.Parent = statusValue

			-- Both glow layers use a brighter version of the accent:
			-- extract mid-point colour from Accents.Main, boost S+V via HSV, reapply
			local function getBrightAccent()
				local seq = Starlight.CurrentTheme.Accents.Main
				local mid = seq.Keypoints[2] and seq.Keypoints[2].Value or seq.Keypoints[1].Value
				local h, s, v = Color3.toHSV(mid)
				return Color3.fromHSV(h, math.min(s * 1.15, 1), math.min(v * 1.25, 1))
			end

			local function applyGlowColour()
				local bright = getBrightAccent()
				glowInner.Color = bright
				glowHalo.ImageColor3 = bright
			end
			applyGlowColour()
			themeEvent.Event:Connect(applyGlowColour)

			-- Align using AbsolutePosition so Y is pixel-perfect regardless of asset layout
			local function alignStatusValue()
				local absPos = subheader.AbsolutePosition
				local parentAbs = subheader.Parent.AbsolutePosition
				statusValue.Position = UDim2.fromOffset(
					(absPos.X - parentAbs.X) + subheader.TextBounds.X + 5,
					(absPos.Y - parentAbs.Y)
				)
			end
			task.defer(alignStatusValue)

			-- Setter: swap text, re-align, pulse glow
			local function setStatus(text)
				statusValue.Text = text
				task.defer(alignStatusValue)
				-- Pulse: glow flares bright then settles back
				Tween(glowInner, { Transparency = 0 }, nil, Tween.Info("Exponential", "Out", 0.12))
				Tween(glowHalo, { ImageTransparency = 0.2 }, nil, Tween.Info("Exponential", "Out", 0.12))
				task.delay(0.35, function()
					Tween(glowInner, { Transparency = 0.1 }, nil, Tween.Info("Exponential", "Out", 0.5))
					Tween(glowHalo, { ImageTransparency = 0.55 }, nil, Tween.Info("Exponential", "Out", 0.5))
				end)
			end
		...
		-- Enable by passing PlayerInfoBlur = true in WindowSettings.
		if WindowSettings.PlayerInfoBlur then
			local playerIcon = mainWindow.Sidebar.Player.PlayerIcon

			-- Blur overlay: stretched image of the blurred asset pre-loaded by Starlight
			...
			blurOverlay.Image = "rbxassetid://18824089198"
			blurOverlay.ImageTransparency = 0.15
			blurOverlay.ScaleType = Enum.ScaleType.Crop
			blurOverlay.ZIndex = playerIcon.ZIndex + 1
			blurOverlay.Parent = playerIcon

			-- Tint overlay: semi-transparent dark layer for the frosted look
			local tintOverlay = Instance.new("Frame")
			tintOverlay.Name = "BlurTint"
			tintOverlay.Size = UDim2.fromScale(1, 1)
			tintOverlay.Position = UDim2.fromScale(0, 0)
			tintOverlay.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
			tintOverlay.BackgroundTransparency = 0.6
			tintOverlay.BorderSizePixel = 0
			tintOverlay.ZIndex = playerIcon.ZIndex + 2
			tintOverlay.Parent = playerIcon

			-- Inherit UICorner from PlayerIcon if it exists, so edges stay rounded
			local existingCorner = playerIcon:FindFirstChildOfClass("UICorner")
			if existingCorner then
				local blurCorner = Instance.new("UICorner")
				blurCorner.CornerRadius = existingCorner.CornerRadius
				blurCorner.Parent = tintOverlay
			end
		end
